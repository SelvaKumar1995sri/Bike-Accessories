...
app = Flask(__name__)
app.secret_key = "SeLvAkUmAr"
app.config["SESSION_COOKIE_NAME"]="mYCoOKIEnamE"
...
def get_db():
    ...
    if db is None:
        db = g._database = sqlite3.connect('accessory_list.db')
        cursor = db.cursor()
        cursor.execute("select name from accessories")
